...
│    │                     │                     │            │
│    ▼                     ▼                     ▼            │
│ Infra Server        Zabbix Server          Хабы            │
│ YOUR_TAILSCALE_IP   YOUR_TAILSCALE_IP  100.64.0.x          │
│                                                             │
│ Весь трафик зашифрован, доступ только изнутри VPN          │
└─────────────────────────────────────────────────────────────┘
                           │
...
│                   ЛОКАЛЬНАЯ СЕТЬ ХАБА                        │
│                                                              │
│    Периферийные устройства подключаются к AP хаба                  │
│    Получают IP из диапазона 192.168.4.x                     │
│    Отдают данные на 192.168.4.1 (хаб)                       │
└─────────────────────────────────────────────────────────────┘
```
...
```
Оператор → Infra Server → Hub → Периферия
              │
              └→ Генерация образов
              └→ Просмотр статуса
              └→ Доступ к терминалам
...
### Мониторинг (снизу вверх)
```
Периферия → Hub → Zabbix Server → Алерты
         │
         └→ Логи устройств
         └→ Статус Wi-Fi сканов
         └→ Метрики системы
...
### Данные (снизу вверх)
```
Периферия (keylog/capture) → Hub (download_log) → Оператор
```
