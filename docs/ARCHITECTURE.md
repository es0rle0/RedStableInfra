# Архитектура Red Team Infrastructure

Система централизованного управления распределёнными атакующими устройствами.

## Обзор

Когда атакующая инфраструктура перестаёт помещаться в рюкзак, ей нужен контроль. Не контроль жертвы — а контроль собственных устройств.

Система позволяет red team команде:
- Генерировать и разворачивать образы устройств
- Мониторить состояние всей инфраструктуры
- Удалённо управлять устройствами через защищённый VPN
- Иерархически организовать устройства через хабы
