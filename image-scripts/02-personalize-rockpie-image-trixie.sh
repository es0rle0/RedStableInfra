#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# 02-personalize-rockpie-image-trixie.sh
# Персонализация образа Rock Pi E (vanilla Debian trixie).
# hostname/pony + zabbix config + tailscale-firstboot key.
#
# Отличия от RPi-версии:
#   - Один rootfs-раздел (p2), нет FAT boot (p1 — raw u-boot)
#   - Нет userconf.txt / ssh файлов в boot
#   - Нет GPIO eth0-guard (Rock Pi E не имеет RPi GPIO header)
#   - resolv.conf может быть dangling symlink
#
# Требует env:
...
LOG_FILE="$LOG_DIR/02-personalize-rockpie.${RUN_ID}.log"
ERR_FILE="$LOG_DIR/02-personalize-rockpie.${RUN_ID}.err"
...
HOST_PWD="$(pwd)"
WORK="$(mktemp -d)"
TMP_IMG="personalize-rockpie.$(date +%s).$$.img"
READY_IMG=""
...
# Rock Pi E: p1 = ext4 rootfs (debootstrap layout) или p2 = ext4 rootfs (johang layout)
ROOTFS_PART=""
for p in "${LOOP}p1" "${LOOP}p2"; do
  if sudo file -sL "$p" 2>/dev/null | grep -q "ext[234]"; then
    ROOTFS_PART="$p"
    break
  fi
done
[[ -n "$ROOTFS_PART" ]] || { echo "[ERROR] Cannot find ext4 rootfs partition" >&2; exit 1; }

sudo mount "$ROOTFS_PART" "$WORK/root"

# Vanilla Debian: resolv.conf может быть dangling symlink
...
# NetworkManager: игнорировать eth0 (управляется скриптами)
...
# eth0-guard (без GPIO — Rock Pi E не имеет RPi GPIO header)
log "Deploying eth0-guard (scripts + service, no GPIO) …"
...
[Unit]
Description=Disable eth0 at boot unless overridden
...
# Нет eth0-guard-watch (GPIO) — разблокировка только через SSH или консоль

log "Personalization finished"
EOS

# Rock Pi E: нет userconf.txt / ssh файлов — пароль уже задан в chroot скрипта 01
...
info "✓ Personalized Rock Pi E image ready: $READY_IMG"
